...
extension UIBezierPath {
 ...
 static func mx_prepare() {
  ...
  func mx_point(atFractionOfLength t: CGFloat) -> CGPoint {
    ...
    func mx_percent(atPoint: CGPoint) -> CGFloat {
        if isEmpty {
            return 0
        }
    
        //let totalLenth = mx_pathLength
        let fraction: CGFloat = 0
        
//        findPathElement(at: atPoint) { element, <#CGFloat#> in
//            fraction = element.
//        }
...
//    func findPathElement(at point: CGPoint, callback: (_ e: BezierPathElement, _ t: CGFloat) -> Void) {
